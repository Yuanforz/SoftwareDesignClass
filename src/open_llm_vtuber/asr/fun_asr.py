...
# Set default ModelScope cache directory BEFORE any imports that might use it
_DEFAULT_CACHE_DIR = os.path.join(os.path.abspath("."), "models")
if 'MODELSCOPE_CACHE' not in os.environ:
    os.environ['MODELSCOPE_CACHE'] = _DEFAULT_CACHE_DIR
...
class VoiceRecognition(ASRInterface):
    def __init__(
        ...
        # Update cache directory for modelscope if provided
        if cache_dir:
            cache_path = os.path.abspath(cache_dir)
            os.environ['MODELSCOPE_CACHE'] = cache_path
            print(f"ModelScope cache directory set to: {cache_path}")
